...
static void bench_ecmult_1(void* arg, int iters) {
    ...
    for (i = 0; i < iters; ++i) {
        rustsecp256k1_v0_4_1_ecmult(&data->output[i], &data->pubkeys_gej[(data->offset1+i) % POINTS], &data->scalars[(data->offset2+i) % POINTS], NULL);
    }
}
...
static void bench_ecmult_1g(void* arg, int iters) {
    ...
    for (i = 0; i < iters; ++i) {
        rustsecp256k1_v0_4_1_ecmult(&data->output[i], NULL, &zero, &data->scalars[(data->offset1+i) % POINTS]);
    }
}
...
static void bench_ecmult_2g(void* arg, int iters) {
    ...
    for (i = 0; i < iters/2; ++i) {
        rustsecp256k1_v0_4_1_ecmult(&data->output[i], &data->pubkeys_gej[(data->offset1+i) % POINTS], &data->scalars[(data->offset2+i) % POINTS], &data->scalars[(data->offset1+i) % POINTS]);
    }
}
...
static void run_ecmult_multi_bench(bench_data* data, size_t count, int includes_g, int num_iters) {
    ...
    for (iter = 0; iter < iters; ++iter) {
        ...
        rustsecp256k1_v0_4_1_ecmult(&data->expected_output[iter], NULL, &zero, &total);
    }
