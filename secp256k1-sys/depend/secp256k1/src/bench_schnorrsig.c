...
void bench_schnorrsig_sign(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        msg[0] = i;
        msg[1] = i >> 8;
        CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(data->ctx, sig, msg, MSGLEN, data->keypairs[i], NULL));
    }
}
...
void bench_schnorrsig_verify(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(data->ctx, data->sigs[i], data->msgs[i], MSGLEN, &pk));
    }
}
...
int main(void) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(data.ctx, sig, msg, MSGLEN, keypair, NULL));
