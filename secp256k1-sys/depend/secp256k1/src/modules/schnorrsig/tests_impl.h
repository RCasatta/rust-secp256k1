...
void run_nonce_function_bip340_tests(void) {
    ...
    * state. */
    ...
    /* Other algo is fine */
    rustsecp256k1_v0_4_1_rfc6979_hmac_sha256_generate(&rustsecp256k1_v0_4_1_test_rng, algo, algolen);
    CHECK(nonce_function_bip340(nonce, msg, msglen, key, pk, algo, algolen, NULL) == 1);

    for (i = 0; i < count; i++) {
        unsigned char nonce2[32];
        uint32_t offset = rustsecp256k1_v0_4_1_testrand_int(msglen - 1);
        ...
        offset = rustsecp256k1_v0_4_1_testrand_int(algolen - 1);
...
void test_schnorrsig_api(void) {
    ...
    rustsecp256k1_v0_4_1_schnorrsig_extraparams extraparams = SECP256K1_SCHNORRSIG_EXTRAPARAMS_INIT;
    rustsecp256k1_v0_4_1_schnorrsig_extraparams invalid_extraparams = {{ 0 }, NULL, NULL};
    ...
    /** main test body **/
    ecount = 0;
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign(none, sig, msg, &keypairs[0], NULL) == 0);
    CHECK(ecount == 1);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign(vrfy, sig, msg, &keypairs[0], NULL) == 0);
    CHECK(ecount == 2);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign(sign, sig, msg, &keypairs[0], NULL) == 1);
    CHECK(ecount == 2);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign(sign, NULL, msg, &keypairs[0], NULL) == 0);
    CHECK(ecount == 3);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign(sign, sig, NULL, &keypairs[0], NULL) == 0);
    CHECK(ecount == 4);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign(sign, sig, msg, NULL, NULL) == 0);
    CHECK(ecount == 5);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign(sign, sig, msg, &invalid_keypair, NULL) == 0);
    CHECK(ecount == 6);

    ecount = 0;
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(none, sig, msg, sizeof(msg), &keypairs[0], &extraparams) == 0);
    CHECK(ecount == 1);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(vrfy, sig, msg, sizeof(msg), &keypairs[0], &extraparams) == 0);
    CHECK(ecount == 2);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(sign, sig, msg, sizeof(msg), &keypairs[0], &extraparams) == 1);
    CHECK(ecount == 2);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(sign, NULL, msg, sizeof(msg), &keypairs[0], &extraparams) == 0);
    CHECK(ecount == 3);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(sign, sig, NULL, sizeof(msg), &keypairs[0], &extraparams) == 0);
    CHECK(ecount == 4);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(sign, sig, NULL, 0, &keypairs[0], &extraparams) == 1);
    CHECK(ecount == 4);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(sign, sig, msg, sizeof(msg), NULL, &extraparams) == 0);
    CHECK(ecount == 5);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(sign, sig, msg, sizeof(msg), &invalid_keypair, &extraparams) == 0);
    CHECK(ecount == 6);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(sign, sig, msg, sizeof(msg), &keypairs[0], NULL) == 1);
    CHECK(ecount == 6);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(sign, sig, msg, sizeof(msg), &keypairs[0], &invalid_extraparams) == 0);
    CHECK(ecount == 7);

    ecount = 0;
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign(sign, sig, msg, &keypairs[0], NULL) == 1);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(none, sig, msg, sizeof(msg), &pk[0]) == 1);
    CHECK(ecount == 0);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(sign, sig, msg, sizeof(msg), &pk[0]) == 1);
    CHECK(ecount == 0);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(vrfy, sig, msg, sizeof(msg), &pk[0]) == 1);
    CHECK(ecount == 0);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(vrfy, NULL, msg, sizeof(msg), &pk[0]) == 0);
    CHECK(ecount == 1);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(vrfy, sig, NULL, sizeof(msg), &pk[0]) == 0);
    CHECK(ecount == 2);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(vrfy, sig, NULL, 0, &pk[0]) == 0);
    CHECK(ecount == 2);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(vrfy, sig, msg, sizeof(msg), NULL) == 0);
    CHECK(ecount == 3);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(vrfy, sig, msg, sizeof(msg), &zero_pk) == 0);
    CHECK(ecount == 4);
...
void test_schnorrsig_bip_vectors_check_signing(const unsigned char *sk, const unsigned char *pk_serialized, unsigned char *aux_rand, const unsigned char *msg32, const unsigned char *expected_sig) {
    ...
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign(ctx, sig, msg32, &keypair, aux_rand));
    ...
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(ctx, sig, msg32, 32, &pk));
}
...
void test_schnorrsig_bip_vectors_check_verify(const unsigned char *pk_serialized, const unsigned char *msg32, const unsigned char *sig, int expected) {
    ...
    CHECK(expected == rustsecp256k1_v0_4_1_schnorrsig_verify(ctx, sig, msg32, 32, &pk));
}
...
void test_schnorrsig_sign(void) {
    ...
    CHECK(rustsecp256k1_v0_4_1_keypair_create(ctx, &keypair, sk));
    CHECK(rustsecp256k1_v0_4_1_keypair_xonly_pub(ctx, &pk, NULL, &keypair));
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign(ctx, sig, msg, &keypair, NULL) == 1);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(ctx, sig, msg, sizeof(msg), &pk));

    /* Test different nonce functions */
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(ctx, sig, msg, sizeof(msg), &keypair, &extraparams) == 1);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(ctx, sig, msg, sizeof(msg), &pk));
    memset(sig, 1, sizeof(sig));
    extraparams.noncefp = nonce_function_failing;
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(ctx, sig, msg, sizeof(msg), &keypair, &extraparams) == 0);
    ...
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(ctx, sig, msg, sizeof(msg), &keypair, &extraparams) == 1);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign(ctx, sig2, msg, &keypair, extraparams.ndata) == 1);
...
void test_schnorrsig_sign_verify(void) {
    ...
    for (i = 0; i < N_SIGS; i++) {
        ...
        CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign(ctx, sig[i], msg[i], &keypair, NULL));
        CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(ctx, sig[i], msg[i], sizeof(msg[i]), &pk));
    }

    {
        ...
        CHECK(!rustsecp256k1_v0_4_1_schnorrsig_verify(ctx, sig[sig_idx], msg[sig_idx], sizeof(msg[sig_idx]), &pk));
        sig[sig_idx][byte_idx] ^= xorbyte;
    ...
    /* Test overflowing s */
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign(ctx, sig[0], msg[0], &keypair, NULL));
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(ctx, sig[0], msg[0], sizeof(msg[0]), &pk));
    ...
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(ctx, sig[0], NULL, 0, &keypair, NULL) == 1);
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(ctx, sig[0], NULL, 0, &pk) == 1);

    {
        /* Test varying message lengths */
        unsigned char msg_large[32 * 8];
        uint32_t msglen  = rustsecp256k1_v0_4_1_testrand_int(sizeof(msg_large));
        for (i = 0; i < sizeof(msg_large); i += 32) {
            rustsecp256k1_v0_4_1_testrand256(&msg_large[i]);
        }
        CHECK(rustsecp256k1_v0_4_1_schnorrsig_sign_custom(ctx, sig[0], msg_large, msglen, &keypair, NULL) == 1);
        CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(ctx, sig[0], msg_large, msglen, &pk) == 1);
        /* Verification for a random wrong message length fails */
        msglen = (msglen + (sizeof(msg_large) - 1)) % sizeof(msg_large);
        CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(ctx, sig[0], msg_large, msglen, &pk) == 0);
    }
}
#undef N_SIGS
...
void test_schnorrsig_taproot(void) {
    ...
    CHECK(rustsecp256k1_v0_4_1_schnorrsig_verify(ctx, sig, msg, sizeof(msg), &output_pk) == 1);
